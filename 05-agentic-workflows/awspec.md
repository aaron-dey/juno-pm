...
| ---- | ------------------------------------------------------------------------ | ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Read the thread + retrieve customer ID and ARR if mentioned.             | slack.reactions(count unique), read-only                     | Agent can READ Slack #escalations + Strategy Knowledge Base + Salesforce ARR.  Agent can WRITE to #pm-daily and create Jira stubs.  Agent CANNOT edit Salesforce records, edit Jira tickets after creation, or post outside #pm-daily. |
...
- salesforce.lookup_arr → {arr_eur, contract_end, churn_risk}.
...
- **External :** 
  In-scope: 
  READ: Slack thread API, RocketShip Strategy Knowledge Base, Salesforce ARR lookup
  WRITE: Slack channel #pm-daily, Jira ticket (write, stub creation only).

## Human-in-the-loop

PM reviews any P0 with confidence < 70% before posting. 
Hourly review: PM prompted every 55 mins if required within working hours (9:00am and 5:00pm)
...
- **Fails safe when:** Agent can READ Slack #escalations + Strategy Knowledge Base + Salesforce ARR. 
